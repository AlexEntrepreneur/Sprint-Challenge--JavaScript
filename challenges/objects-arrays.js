...
console.log(`Stegosaurus Length: ${steg.length}`);
...
console.log(`trex.roar() => ${trex.roar()}`);
...
const uni = [];
for (let graduate of graduates) {
  let containsUni = graduate.university.indexOf("Uni");
  if (containsUni > -1) {
...
populationTotal = zooAnimals.reduce((accumulator, currAnimal) => {
  return accumulator += currAnimal.population;
}, 0);
